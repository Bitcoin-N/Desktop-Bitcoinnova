...
namespace TurtleWallet
{
    public partial class wallet : Form
    {
        ...
        private void Wallet_Load(object sender, EventArgs e)
        {
            ...
            windowLogger.Log(LogTextbox, "bitcoin nova Wallet " + "v" + System.Reflection.Assembly.GetExecutingAssembly().GetName().Version + " has started ...");
        ...
        private void Refresh_ui()
        {
            ...
            try
            {
                ...
                float availableBal = (float)(balances.Item3["availableBalance"]) / 1000000;
                float lockedBal = (float)(balances.Item3["lockedAmount"]) / 1000000;
                this.availableAmountLabel.BeginInvoke((MethodInvoker)delegate () { this.availableAmountLabel.Text = availableBal.ToString("0.00") + " BTN"; });
                this.lockedAmountLabel.BeginInvoke((MethodInvoker)delegate () { this.lockedAmountLabel.Text = lockedBal.ToString("0.00") + " BTN"; });
            ...
            catch (Exception)
            {
                ...
                if (currentTimeout >= watchdogTimeout)
                {
                    ...
                    {
                        MessageBox.Show("bitcoin nova Wallet has tried numerous times to relaunch the needed daemon and has failed. Please relaunch the wallet!", "Walletd daemon could not be recovered!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        windowLogger.Log(LogTextbox, "bitcoin nova Wallet has tried numerous times to relaunch the needed daemon and has failed. Please relaunch the wallet!");
        ...
        private void CopyAddressButton_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show("Address copied to clipboard!", "bitcoin nova Wallet");
        }
        ...
        {
            DialogResult dialogResult = MessageBox.Show("Are you sure you want to close bitcoin nova Wallet?", "bitcoin nova Wallet", MessageBoxButtons.YesNo);

            if (dialogResult == DialogResult.Yes)
            {
                updateLabel.Text = "Saving wallet, Please wait..";
                saverWorker.RunWorkerAsync();
                MessageBox.Show("Saving Wallet, Please wait...", "bitcoin nova Wallet");
            }
        ...
        private void SendTrtlButton_Click(object sender, EventArgs e)
        {
            ...
            if(sendAddr == myAddr)
            {
                MessageBox.Show("Sending to yourself is not supported.", "bitcoin nova Wallet", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            ...
            try
            {
                ...
                if(resp.Item1 == false)
                {
                    MessageBox.Show("Error occured on send:" + Environment.NewLine + resp.Item2, "bitcoin nova Wallet", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ...
                MessageBox.Show("Transaction send was successful!" + Environment.NewLine + "Amount: " + ((float)amount / 1000000).ToString() + Environment.NewLine + "Mix: " + mixins.ToString() + Environment.NewLine + "To: " + sendAddr + Environment.NewLine + "Trx hash: " + txhash, "bitcoin nova Wallet", MessageBoxButtons.OK,MessageBoxIcon.Information);
                windowLogger.Log(LogTextbox, "Transaction send was successful!" + Environment.NewLine + "Amount: " + ((float)amount / 1000000).ToString() + Environment.NewLine + "Mix: " + mixins.ToString() + Environment.NewLine + "To: " + sendAddr + Environment.NewLine + "Trx hash: " + txhash);
            }
            catch(Exception ex)
            {
        ...
        private void SendRPCButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Invalid method on RPC send", "bitcoin nova Wallet", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            if (argTextbox.Text == "")
            {
                MessageBox.Show("Invalid argument on RPC send. If there are no arguments, use '{}'.", "bitcoin nova Wallet", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            try
            {
        ...
        private void BackupSubmitbutton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please enter a valid password.", "bitcoin nova Wallet");
                return;
            }
            if(backupPasswordText.Text != WalletPassword)
            {
                MessageBox.Show("Incorrect password!", "bitcoin nova Wallet");
                return;
            }
            ...
            {
                MessageBox.Show("Error occured on getViewKey:" + Environment.NewLine + viewresp.Item2, "bitcoin nova Wallet", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            {
                MessageBox.Show("Error occured on getSpendKeys:" + Environment.NewLine + spendresp.Item2, "bitcoin nova Wallet", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            MessageBox.Show("Wallet keys successfully unlocked!", "bitcoin nova Wallet");
        }
        ...
        private void SaverWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Error occured trying to save the wallet state:" + Environment.NewLine + saveresp.Item2, "bitcoin nova Wallet", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch { }
