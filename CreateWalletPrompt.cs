...
namespace TurtleWallet
{
    public partial class CreateWalletPrompt : Form
    {
        ...
        {
            DialogResult dialogResult = MessageBox.Show("Are you sure you want to cancel your bitcoin nova Wallet creation?", "Cancel wallet creation?", MessageBoxButtons.YesNo);
        ...
        private void CreateWallet()
        {
            ...
            {
                MessageBox.Show("Please enter a valid wallet name", "bitcoin nova Wallet Creation");
                return;
            }
            else if (walletNameText.Text.Any(c => !Char.IsLetterOrDigit(c)))
            {
                MessageBox.Show("Wallet name cannot contain special characters", "bitcoin nova Wallet Creation");
                return;
            }
            else if (System.IO.File.Exists(_walletFile))
            {
                MessageBox.Show("A wallet with that name already exists! Choose a different name or choose the \"Select Existing Wallet\" option instead.", "bitcoin nova Wallet Creation");
                return;
            }

            if (passwordText.Text == "")
            {
                MessageBox.Show("Please enter a valid password", "bitcoin nova Wallet Creation");
                return;
            }
            else if (passwordText.Text.Length < 6)
            {
                MessageBox.Show("Please enter a password that is larger than 6 characters", "bitcoin nova Wallet Creation");
                return;
            }
            else if (passwordText.Text.Length > 150)
            {
                MessageBox.Show("Passwords cannot be longer than 150 characters!", "bitcoin nova Wallet Creation");
                return;
            }

            if (passwordText.Text != passwordConfirmText.Text)
            {
                MessageBox.Show("Passwords do not match", "bitcoin nova Wallet Creation");
                return;
            }
            ...
            {
                MessageBox.Show("The 'walletd' daemon is missing from the folder the wallet is currently running from! Please place 'walletd' next to your wallet exe and run again!", "bitcoin nova Wallet Creation", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            try
            {
                ...
                {
                    MessageBox.Show("Wallet failed to create after communicating with daemon. Please reinstall the wallet, close any other wallets you may have open, and try again", "bitcoin nova Wallet Creation", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Utilities.SetDialogResult(this, DialogResult.Abort);
                    Utilities.Close(this);
                }
                else
            ...
            {
                MessageBox.Show("An exception occured while attempting to create the wallet." + Environment.NewLine + "Error:" + Environment.NewLine + ex.Message, "bitcoin nova Wallet Creation", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Utilities.SetDialogResult(this, DialogResult.Abort);
                Utilities.Close(this);
            }
        }
